...
export default function InstallPrompt() {
    ...
    return (
      <div className="card flex items-center gap-3 border-accent-500/40 bg-accent-500/5">
        ...
        <div className="flex-1">
          <div className="font-semibold text-sm">{t('install.android.title')}</div>
  ...
  return (
    <div className="card flex items-center gap-3 border-accent-500/30 bg-accent-500/5">
      <div className="text-3xl">📲</div>
      <div className="flex-1">
        <div className="font-semibold text-sm">{t('install.ios.title')}</div>
        <div className="text-xs text-slate-400">
          {t('install.ios.body_before')}
          <strong className="text-slate-200">{t('install.ios.body_action')}</strong>
          {t('install.ios.body_after')}
        </div>
      </div>
      <button
        onClick={dismiss}
